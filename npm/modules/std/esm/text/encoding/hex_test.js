...
dntShim.Deno.test("[encoding.hex] encode", () => {
    {
        const srcStr = "abc";
...
dntShim.Deno.test("[encoding.hex] decode", () => {
...
dntShim.Deno.test("[encoding.hex] decode error", () => {
    for (const [input, expectedErr, msg] of errCases) {
        assertThrows(() => decode(new TextEncoder().encode(input)), expectedErr, msg);
    }
});
