...
export function relative(from, to) {
    ...
    for (; toStart < toEnd; ++toStart) {
        if (to.charCodeAt(toStart) !== CHAR_BACKWARD_SLASH && to.charCodeAt(toStart) !== CHAR_FORWARD_SLASH)
            break;
    }
    // Trim trailing backslashes (applicable to UNC paths only)
    for (; toEnd - 1 > toStart; --toEnd) {
        if (to.charCodeAt(toEnd - 1) !== CHAR_BACKWARD_SLASH && to.charCodeAt(toEnd - 1) !== CHAR_FORWARD_SLASH)
            break;
    }
    ...
    for (; i <= length; ++i) {
        ...
        if (fromCode !== toCode)
            break;
        else if (fromCode === CHAR_BACKWARD_SLASH || fromCode === CHAR_FORWARD_SLASH)
            lastCommonSep = i;
    }
