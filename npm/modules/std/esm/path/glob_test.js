...
import { SEP } from "./mod.js";
function match(glob, path, opts = {}) {
    if (opts.os == null) {
        const matchDarwin = path.match(globToRegExp(glob, { ...opts, os: "darwin" }));
        if (matchDarwin) {
        ...
        const matchWindows = path.match(globToRegExp(glob, { ...opts, os: "windows" }));
        if (matchWindows) {
...
dntShim.Deno.test({
    ...
    fn() {
        assert(match("[[:alnum:]]/bar.txt", "a/bar.txt", { extended: false, globstar: false }));
        assert(match("[[:alnum:]abc]/bar.txt", "1/bar.txt", { extended: false, globstar: false }));
        assert(!match("[[:alnum:]]/bar.txt", "!/bar.txt", { extended: false, globstar: false }));
...
dntShim.Deno.test({
    ...
    fn() {
        assert(match("foo{bar,baaz}", "foobaaz", { extended: false, globstar: false }));
        assert(match("foo{bar,baaz}", "foobar", { extended: false, globstar: false }));
        assert(!match("foo{bar,baaz}", "foobuzz", { extended: false, globstar: false }));
        assert(match("foo{bar,b*z}", "foobuzz", { extended: false, globstar: false }));
    },
});
dntShim.Deno.test({
    name: "[path] globToRegExp() Complex matches",
    fn() {
        assert(match("http://?o[oz].b*z.com/{*.js,*.html}", "http://foo.baaz.com/jquery.min.js", { extended: false, globstar: false }));
        assert(match("http://?o[oz].b*z.com/{*.js,*.html}", "http://moz.buzz.com/index.html", { extended: false, globstar: false }));
        assert(!match("http://?o[oz].b*z.com/{*.js,*.html}", "http://moz.buzz.com/index.htm", { extended: false, globstar: false }));
        assert(!match("http://?o[oz].b*z.com/{*.js,*.html}", "http://moz.bar.com/index.html", { extended: false, globstar: false }));
        assert(!match("http://?o[oz].b*z.com/{*.js,*.html}", "http://flozz.buzz.com/index.html", { extended: false, globstar: false }));
    },
});
dntShim.Deno.test({
    ...
    fn() {
        ...
        assert(!match("/foo/**.txt", "/foo/bar/baz/qux.txt"));
        assert(!match("/foo/bar**/*.txt", "/foo/bar/baz/qux.txt"));
        assert(!match("/foo/bar**", "/foo/bar/baz.txt"));
        assert(!match("**/.txt", "/foo/bar/baz/qux.txt"));
        assert(!match("http://foo.com/*", "http://foo.com/bar/baz/jquery.min.js"));
        assert(!match("http://foo.com/*", "http://foo.com/bar/baz/jquery.min.js"));
        assert(match("http://foo.com/**", "http://foo.com/bar/baz/jquery.min.js"));
        assert(match("http://foo.com/**/jquery.min.js", "http://foo.com/bar/baz/jquery.min.js"));
        assert(!match("http://foo.com/*/jquery.min.js", "http://foo.com/bar/baz/jquery.min.js"));
    },
});
dntShim.Deno.test({
...
dntShim.Deno.test({
    name: "[path] globToRegExp() Special extended characters should match themselves",
    fn() {
        const glob = "\\/$^+.()=!|,.*";
...
dntShim.Deno.test("[path] normalizeGlob() Globstar", function () {
    assertEquals(normalizeGlob(`**${SEP}..`, { globstar: true }), `**${SEP}..`);
});
dntShim.Deno.test("[path] joinGlobs() Globstar", function () {
