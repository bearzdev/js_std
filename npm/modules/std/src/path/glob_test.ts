...
import { globToRegExp, GlobToRegExpOptions, isGlob, joinGlobs, normalizeGlob } from './glob.js';
...
dntShim.Deno.test('[path] normalizeGlob() Globstar', function (): void {
    assertEquals(normalizeGlob(`**${SEP}..`, { globstar: true }), `**${SEP}..`);
});

dntShim.Deno.test('[path] joinGlobs() Globstar', function (): void {
