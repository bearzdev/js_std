import { DirectoryNotFoundError, InvalidOperationError, env, isDirectory, isWindows, join, DIRECTORY_SEPARATOR, registerExecutable, run, isDirectoryAsync, runAsync, } from '../deps.js';
...
export async function nvsUseAsync(version, arch = 'x64') {
...
export function nvsUse(version, arch = 'x64') {
    const nvsHome = getNvsHome();
    if (!nvsHome) {
        throw new InvalidOperationError("NVS_HOME is not set.");
    }
...
export function nvsAdd(version) {
    return run('nvs', ['add', version]);
}
export function nvsAddAsync(version) {
    return runAsync('nvs', ['add', version]);
}
