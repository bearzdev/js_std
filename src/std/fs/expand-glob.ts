import {
  ...
  import { walk, walkAsync } from "./walk.ts";
  import { isWindows } from "../runtime/mod.ts";
  import { createWalkEntry, createWalkEntryAsync, WalkEntry } from "./_util.ts";
import { notNull } from "../errors/check.ts";
import { cwd } from "../env/current-directory.ts";
import { DirectoryNotFoundError, FileNotFoundError } from "./errors.ts";
  ...
  function throwUnlessNotFound(error: unknown): void {
    if (!(error instanceof DirectoryNotFoundError) || !(error instanceof FileNotFoundError)) {
      throw error;
    }
  }
   ...
   * ```
   */
  export async function* expandGlobAsync(
    glob: string,
    {
      root = cwd(),
      exclude = [],
    ...
    try {
      fixedRootInfo = await createWalkEntryAsync(fixedRoot);
    } catch (error) {
      return throwUnlessNotFound(error);
    }
  
    async function* advanceMatchAsync(
      walkInfo: WalkEntry,
      globSegment: string,
    ): AsyncIterableIterator<WalkEntry> {
      if (!walkInfo.isDirectory) {
        ...
        try {
          if (shouldInclude(parentPath)) {
            return yield await createWalkEntryAsync(parentPath);
          }
        } catch (error) {
        ...
        return yield* walkAsync(walkInfo.path, { skip: excludePatterns });
      }
      const globPattern = globToRegExp(globSegment, globOptions);
      for await (
        const walkEntry of walkAsync(walkInfo.path, {
          maxDepth: 1,
    ...
    for (const segment of segments) {
      ...
      await Promise.all(currentMatches.map(async (currentMatch) => {
        for await (const nextMatch of advanceMatchAsync(currentMatch, segment)) {
  ...
  export function* expandGlob(
    ...
    let fixedRootInfo: WalkEntry;
    try {
      fixedRootInfo = createWalkEntry(fixedRoot);
    } catch (error) {
      return throwUnlessNotFound(error);
    }
    ...
    function* advanceMatch(
      ...
      if (!walkInfo.isDirectory) {
        ...
        try {
          if (shouldInclude(parentPath)) {
            return yield createWalkEntry(parentPath);
          }
        } catch (error) {
          throwUnlessNotFound(error);
        }
