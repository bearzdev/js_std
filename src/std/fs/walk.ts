...
import { lstat, lstatAsync, readDirectory, readDirectoryAsync, realPath, realPathAsync } from "./base.deno.ts";
...
export async function* walkAsync(
  ...
  if (includeDirs && include(root, exts, match, skip)) {
    yield await createWalkEntryAsync(root);
  }
  if (maxDepth < 1 || !include(root, undefined, undefined, skip)) {
    return;
  }
  try {
    for await (const entry of readDirectoryAsync(root)) {
      notNull(entry.name, 'entry.name');
      ...
      if (isSymlink) {
        ...
        ({ isSymlink, isDirectory } = await lstatAsync(path));
      }

      if (isSymlink || isDirectory) {
        yield* walkAsync(path, {
          maxDepth: maxDepth - 1,
          includeFiles,
          includeDirs,
          followSymlinks,
...
export function* walk(
  ...
  for (const entry of entries) {
    ...
    if (isSymlink) {
      ...
      // resolves symlinks but another process can replace the file system
      // entity with a different type of entity before we call lstat().
      ({ isSymlink, isDirectory } = lstat(path));
    }
